  ...
  return (
    <div className="w-full sm:w-[600px]  mx-auto mt-4 bg-blue-50 dark:bg-slate-800 py-2 rounded-lg">
      ...
      <div className="flex flex-row flex-wrap">
        {randomTech?.map((item) => (
          <TechnologyCard key={item.name} item={item} show={show} handleClick={handleClick} disabled={!name} />
        ))}
      </div>

      <div className="m-8 flex flex-col justify-center">
        <Formik initialValues={initialValue.tech} validationSchema={TechValidator} onSubmit={handleSubmit}>
          {(props) => {
